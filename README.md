...
If the display looks wrong, you can inspect SDRAM contents using `tools/test_intel_master.tcl` over JTAG.
...
### Keyboard input via JTAG

The keyboard peripheral accepts scan codes injected over JTAG (no PS/2 hardware needed).
Use the desktop bridge script to type on your PC keyboard and relay to the FPGA:
...
The SoC exposes a keyboard MMIO peripheral at `0x4000_0000`, driven via JTAG injection — no external PS/2 hardware is needed.

The FPGA image includes a reserved JTAG write address `0x4FFF_FF00`. Writing a PS/2 scan code to that address directly asserts `key_valid` in the keyboard peripheral. The JTAG master is wired in `rtl/soc/top_fpga.sv`.
...
That script maps each physical key to its PS/2 make-code and injects it via JTAG.
